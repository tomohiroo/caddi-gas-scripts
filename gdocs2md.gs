...
function makeMdFiles() {
  var folderId = "17-bETa3BeE-OchZqXvaET-MA1k3fYYel";          // ここを変更する
  var folder = DriveApp.getFolderById(folderId);
  var files = folder.getFiles();
  var mdFolder = folder.createFolder("md");

  while(files.hasNext()) {
    var file = files.next();
    var fileId = file.getId();
    if (fileId == "1CXKl9TMtlMnX7UvnHbmezdI7ITNsTdScLlYSTHvIeUxgSo4twkNQf8Bs") {
      continue;
    }
    convertToMarkdown(DocumentApp.openById(file.getId())).forEach(function(f) {
      mdFolder.createFile(f.fileName, f.content, f.mimeType);
    })
  }
}
